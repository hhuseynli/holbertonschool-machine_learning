#!/usr/bin/env python3
""" Module containing a Neural Network with training visualization """
...
class NeuralNetwork:
    ...
    def evaluate(self, X, Y):
        ...
        Returns:
            The prediction matrix and its cost
        """
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05, verbose=True,
              graph=True, step=100):
        """ Trains the neural network using forward prop and gradient descent
        Args:
            X => ndarray (nx, m) containing the input data
            Y => ndarray (1, m) containing the correct labels
            iterations => number of iterations to train over
            alpha => learning rate
            verbose => boolean defining whether to print log values
            graph => boolean defining whether to generate matplotlib graph
            step => iteration step intervals for verbose/graph updates
        Returns:
            The evaluation of the training data after training completes.
        """
        # 1. Iterations type check
        if not isinstance(iterations, int):
            raise TypeError("iterations must be an integer")
        # 2. Iterations value check
        if iterations <= 0:
        ...
        # 5. Step validations (only if verbose or graph is True)
        if verbose or graph:
        ...
        # Arrays to collect graph plotting milestones
        steps_collected = []
        costs_collected = []
        ...
        for i in range(iterations):
            ...
            # Milestones check (0th iteration and subsequent steps)
            if (verbose or graph) and (i % step == 0):
                ...
                if verbose:
                    print("Cost after {} iterations: {}".format(i,
                          current_cost))
                if graph:
                    steps_collected.append(i)
                    costs_collected.append(current_cost)

            self.gradient_descent(X, Y, A1, A2, alpha)

        # Final iteration tracking (after loop completes)
        final_cost = self.cost(Y, self.__A2)
        if verbose:
            print("Cost after {} iterations: {}".format(iterations,
                  final_cost))
        if graph:
            steps_collected.append(iterations)
            costs_collected.append(final_cost)

            # Plot configuration
            plt.plot(steps_collected, costs_collected, 'b-')
